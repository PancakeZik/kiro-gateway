...
def get_truncation_recovery_system_addition() -> str:
    ...
    return (
        ...
        "will be truncated and FAIL — the content is lost entirely and must be regenerated. "
        "No single Write or Edit tool call may exceed 100 lines. This is a hard limit — calls over 100 lines WILL be truncated and fail. "
        "You MUST break large changes into multiple sequential calls:\n"
        "- When creating files over ~100 lines, write the first ~100 lines with Write, then use Edit to append the rest in chunks of ≤100 lines.\n"
        "- When replacing large blocks with Edit, break the replacement into multiple smaller Edit calls. "
        "Each Edit's new_string MUST be under 100 lines. No exceptions — split into sequential edits.\n"
        "- Never attempt to write an entire large file in a single tool call. Always chunk.\n\n"
        "IMPORTANT: The API has a hard limit of 100 images per message turn. "
        "If you need to read more than 100 images, split them across multiple turns."
    )
...
def convert_images_to_kiro_format(
    ...
    # Kiro API limit: max 100 images per message turn (tested 2026-06-21)
    KIRO_MAX_IMAGES_PER_TURN = 100
...
def build_kiro_history(
    ...
    for msg in messages:
        if msg.role == "user":
            ...
            # Include images in-context — placed at the history turn where they
            # originally appeared so the model sees them in the right position
            # rather than thinking they were all sent in the current message.
            msg_images = msg.images or extract_images_from_content(msg.content)
            if msg_images:
                kiro_images = convert_images_to_kiro_format(msg_images)
                if kiro_images:
...
def build_kiro_payload(
    ...
    # and create a synthetic user message to carry tool results
    ...
    # If content is empty - use a neutral system marker instead of "Continue"
    # to avoid the model interpreting it as user approval/intent
    ...
    # Attach images that belong to the current message only
    current_images = current_message.images or extract_images_from_content(current_message.content)
    if current_images:
    ...
    # Payload size limit: Kiro API rejects requests over ~35MB total.
    # If over budget, drop images from oldest history turns first.
    MAX_PAYLOAD_BYTES = 30_000_000  # 30MB conservative limit
    payload_json = json.dumps(payload)
    if len(payload_json) > MAX_PAYLOAD_BYTES and history:
        logger.warning(
            f"Payload size {len(payload_json)/1_000_000:.1f}MB exceeds {MAX_PAYLOAD_BYTES/1_000_000:.0f}MB limit. "
            f"Dropping images from oldest history turns."
        )
        for turn in history:
            if len(payload_json) <= MAX_PAYLOAD_BYTES:
                break
            user_msg = turn.get("userInputMessage", {})
            if "images" in user_msg:
                del user_msg["images"]
                payload_json = json.dumps(payload)
        if len(payload_json) > MAX_PAYLOAD_BYTES:
            logger.warning(
                f"Payload still {len(payload_json)/1_000_000:.1f}MB after dropping all history images."
            )
