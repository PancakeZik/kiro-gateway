...
from kiro.converters_openai import (_extract_images_from_tool_message,
                                    build_kiro_payload,
...
class TestBuildKiroPayload:
    ...
    def test_handles_assistant_as_last_message(self):
        ...
        Purpose: Ensure neutral system marker is created instead of user-like message.
        """
        ...
        assert current_content == "[Tool results provided, continue processing]"
    ...
    def test_uses_neutral_marker_for_empty_content(self):
        """
        What it does: Verifies using neutral system marker for empty content.
        Purpose: Ensure empty message is replaced with a system-style marker
        that the model won't interpret as user approval/intent.
        """
    ...
    def test_injects_thinking_tags_even_when_tool_results_present(self):
        ...
        assert (
            "<thinking_mode>enabled</thinking_mode>" in content
        ), "thinking tags SHOULD be injected even with toolResults"
        assert (
            "<max_thinking_length>4000</max_thinking_length>" in content
